...
/* Host memory a tensor read lands in at the device's full speed (pinned where
 * the backend copies through a bounce buffer otherwise); NULL on failure. */
...
/* Reserve one device arena for the model ranges about to be cached (their
 * total, each rounded to 256 bytes); returns 0 when the cache path does not
 * apply, and the ranges then fall back to per-chunk arenas. */
int ds4_gpu_reserve_model_cache(uint64_t bytes);
...
int ds4_gpu_qwen4exp_hc_gate(
        ...
        uint32_t              out_dim,
        uint32_t              rows);
int ds4_gpu_qwen4exp_hc_gate_mix(
        ds4_gpu_tensor       *mixed,
        const void           *model_map,
        uint64_t              model_size,
        uint64_t              weight_offset,
